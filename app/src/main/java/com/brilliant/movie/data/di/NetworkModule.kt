...
object NetworkModule {
    ...
    fun provideCache(app: Application): Cache {
        return Cache(app.cacheDir, Constants.CACHE_SIZE_BYTES)
    }
    ...
    fun provideOfflineInterceptor(connectivityManager: ConnectivityManager): Interceptor {
        return Interceptor { chain ->
            ...
            if (!isNetworkAvailable(connectivityManager)) {
                val maxStale = TimeUnit.DAYS.toSeconds(Constants.CACHE_MAX_STALE_DAYS).toInt()
    ...
    fun provideNetworkInterceptor(): Interceptor {
        return Interceptor { chain ->
            ...
            val maxAge = TimeUnit.MINUTES.toSeconds(Constants.CACHE_MAX_AGE_MINUTES).toInt()
            response.newBuilder()
